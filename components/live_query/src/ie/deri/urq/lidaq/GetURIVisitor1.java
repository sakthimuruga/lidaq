 ...
 * @date Jun 13, 2011
 */
public class GetURIVisitor1 implements OpVisitor {
	
	private final Set<Resource> s_uris= new HashSet<Resource>();
	private final Set<Resource> o_uris= new HashSet<Resource>();
	private final Set<Resource> p_uris= new HashSet<Resource>();
	/* (non-Javadoc)
	...
	public void visit(OpBGP opBGP) {
		...
		for(Triple t: opBGP.getPattern()){
			if(t.getSubject() instanceof Node_URI){
				Resource v = new Resource(t.getSubject().toString());
				s_uris.add(v);
			}
			if(t.getPredicate() instanceof Node_URI){
				Resource v = new Resource(t.getPredicate().toString());
				p_uris.add(v);
			}
			if(t.getObject() instanceof Node_URI){
				Resource v = new Resource(t.getObject().toString());
				o_uris.add(v);
			}
	...
	public Set<Resource> getPredicateURIs() {
		return p_uris;
	}
	public Set<Resource> getObjectURIs() {
		return o_uris;
	}
