...
public class AggQueryStats2{
	 ...
	 * 
	 */
	public AggQueryStats2(String queryClass) {
		qc = queryClass;
	}
	...
	Map<String,DescriptiveStatistics[]> termCounts = new TreeMap<String, DescriptiveStatistics[]>();
	Map<String,DescriptiveStatistics[]> setupValues = new TreeMap<String, DescriptiveStatistics[]>();
	Map<String,DescriptiveStatistics[]> relativeSetupValues = new TreeMap<String, DescriptiveStatistics[]>();
	int success = 0;
	int errors = 0;
	...
	public void update(QueryStats1 value) {
		...
		for(Entry<File,Integer[]> ent : value.setupValues.entrySet()){
			...
			String key = ent.getKey().getName().substring(ent.getKey().getName().lastIndexOf("."));
			DescriptiveStatistics [] counts = termCounts.get(key);
			DescriptiveStatistics [] aggAbsV = setupValues.get(key);
			DescriptiveStatistics [] aggRelV = relativeSetupValues.get(key);

			if(counts == null){
				counts = new DescriptiveStatistics[2];
				counts[0]= new DescriptiveStatistics();
				counts[1]= new DescriptiveStatistics();
				termCounts.put(key, counts);
			}
			...
			if(aggRelV == null){
				...
				relativeSetupValues.put(key, aggRelV);
			}
			for(int i =0; i< aggRelV.length; i++){
				...
				if(ent.getKey().equals(baseFile)){
					...
					if(baseValue[i] != 0){
						double valu = 100*((double)(qV[i])/(double)baseValue[i]);
						if(i == (QueryExecutionBenchmark.summaryOrder.indexOf(QueryExecutionBenchmark.TOTAL_TIME)-1)){
	...
	public void update(QueryStats value, boolean onlyStable) {
		if(onlyStable && !value.isStable() && value.emptyResults()){
			...
			for(Entry<File,Integer[]> ent : value.setupValues.entrySet()){
				...
				if(aggV == null){
					aggV = new DescriptiveStatistics[qV.length];
					relativeSetupValues.put(key, aggV);
				}
				for(int i =0; i< aggV.length; i++){
					if(aggV[i]==null) aggV[i]=new DescriptiveStatistics();
					if(ent.getKey().getName().contains("2smart")){
	...
	public String toTexTable(){
		try{
			...
			for(Entry<String, DescriptiveStatistics[]> ent: setupValues.entrySet()){
				...
				String name = ent.getKey().replaceAll("[0-9]","");
				...
				sb.append("& ").append(twoDForm.format(values[i].getMean()/1000D));
				sb.append("& ").append(twoDForm.format(values[i].getStandardDeviation()/1000D));
	...
	public String toTermTable(){
		try{
			...
			//				sb.append("\n").append("\\multicolumn{13}{l}{Query class \\textbf{"+qc+"} with "+success+" queries}\\\\\\hline");
			for(Entry<String, DescriptiveStatistics[]> ent: termCounts.entrySet()){
				DescriptiveStatistics [] values = setupValues.get(ent.getKey());
				String name = ent.getKey().replaceAll("[0-9]","");
				sb.append(QueryStats1.nameMapping.get(name));
				int i =QueryExecutionBenchmark.summaryOrder.indexOf(QueryExecutionBenchmark.TOTAL_RESULTS)-1;
				sb.append("& ").append(twoDForm.format(values[i].getMean()));
				...
				values = ent.getValue();
//				sb.append("& ").append(twoDForm.format(values[0].getMean()));
//				sb.append("& ").append(twoDForm.format(values[0].getStandardDeviation()));
...
//					sb.append("& ").append("-");
	...
	public String toRecallTexTable(){
		try{
			 ...
			 */
			for(Entry<String, DescriptiveStatistics[]> ent: relativeSetupValues.entrySet()){
				if(ent.getKey().contains("smart")){
	...
	private void addRecallData(StringBuilder sb, String key) {
		String name = key.replaceAll("[0-9]","");
	...
	private void addCombinedValues(StringBuilder sb,
			Entry<String, DescriptiveStatistics[]> ent, DescriptiveStatistics[] absolute) {
		DescriptiveStatistics [] values = ent.getValue();
		String name = ent.getKey().replaceAll("[0-9]","");
	...
	private void addDataValues(StringBuilder sb,
			Entry<String, DescriptiveStatistics[]> ent,int c,NumberFormat format) {
		DescriptiveStatistics [] values = ent.getValue();
		String name = ent.getKey().replaceAll("[0-9]","");
...
//		
	...
	private void addValues(StringBuilder sb,
			Entry<String, DescriptiveStatistics[]> ent) {
		DescriptiveStatistics [] values = ent.getValue();
		String name = ent.getKey().replaceAll("[0-9]","");
